...
const CATEGORY_COLORS: Record<UnitCategory, string> = {
  ...
  WHERE: 'bg-cat-where',
  WHEN: 'bg-cat-when',
  SPECIFIC: 'bg-cat-specific',
};
const CATEGORY_TEXT: Record<UnitCategory, string> = {
  WHO: 'text-cat-who',
  WHAT: 'text-cat-what',
  WHERE: 'text-cat-where',
  WHEN: 'text-cat-when',
  SPECIFIC: 'text-cat-specific',
};
...
export default function RecallScoring() {
  ...
  // Set recall start time on mount
  useEffect(() => {
    if (!state.recallStartTime) {
      setRecallStartTime(new Date().toISOString());
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const check = () => {
      if (Date.now() - lastTapRef.current >= 20000 && !promptDismissed) {
        setShowPrompt(true);
      }
    };
  ...
  const handleDismissPrompt = () => {
    ...
    setShowPrompt(false);
    setOneTimePromptUsed();
  };

  const handleToggleTimer = () => {
    if (!timerEnabled) {
      setTimerEnabled(true);
      setRecallTimerUsed(true);
    }
  };
        ...
        <div className="flex items-center justify-between max-w-3xl mx-auto">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              SAY: "Tell me everything you remember from the passage."
            </p>
          </div>
      ...
      <div className="flex-1 px-6 py-6 max-w-3xl mx-auto w-full space-y-6">
            ...
            <div key={category}>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className={`h-full ${CATEGORY_COLORS[category]} transition-all duration-300 rounded-full`}
                    style={{ width: `${(recalled / total) * 100}%` }}
                  />
                </div>
                <span className="text-sm font-medium text-muted-foreground">
                  {recalled} / {total} {complete && '✓'}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {catUnits.map(unit => (
                  <UnitTile
                    key={unit.unit_id}
                    unit={unit}
                    isRecalled={state.recalledUnits.has(unit.unit_id)}
                    isLocked={complete && !state.recalledUnits.has(unit.unit_id)}
                    onTap={handleTap}
                    category={category}
                  />
                ))}
        ...
        {showPrompt && !promptDismissed && (
          <motion.div
            ...
            className="card-elevated border-2 animate-pulse-border p-4 flex items-start gap-3"
          >
            ...
            <Button variant="ghost" size="sm" onClick={handleDismissPrompt}>
              Used — dismiss
            </Button>
          </motion.div>
        )}
      ...
      <div className="sticky bottom-0 bg-background border-t px-6 py-4">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <span className="text-display text-xl text-foreground">
            Score so far: <span className="text-primary">{totalRecalled}</span> / 20
          </span>
          <Button variant="hero" size="xl" onClick={() => goToScreen(6)}>
            Recall Complete
          </Button>
        </div>
      </div>
...
function UnitTile({
  ...
  const handleClick = () => {
    ...
    if (isRecalled && now - lastTapTime.current < 400) {
      onTap(unit.unit_id);
    } else if (!isRecalled && !isLocked) {
      onTap(unit.unit_id);
    }
  ...
  return (
    <button
      onClick={handleClick}
      className={`min-h-[60px] px-3 py-2 rounded-lg flex items-center justify-center gap-2 transition-all active:scale-95 tap-target ${
        isRecalled
          ? `${CATEGORY_COLORS[category]} text-primary-foreground font-medium shadow-md`
