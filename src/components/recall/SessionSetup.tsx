...
export default function SessionSetup() {
  const { state, setParticipant, goToScreen } = useRecall();
  ...
  const handleNewParticipant = () => {
    if (state.isPractice) {
      const practiceP: ParticipantRecord = {
        ...
        created_by_facilitator: state.facilitator?.id || '',
        created_at_location: state.location,
        session_count: 0,
          ...
          <motion.div key="returning" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-md space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                placeholder="NNNN"
                value={numInput}
                onChange={e => setNumInput(e.target.value.replace(/\D/g, '').slice(0, 4))}
                className="w-24 h-12 text-center text-lg"
                inputMode="numeric"
              />
            </div>
            ...
            <Button
              ...
              size="xl"
              className="w-full"
              disabled={yearInput.length !== 4 || numInput.length === 0}
            ...
            <div className="border-t pt-4 text-center">
              ...
              <Button variant="ghost" onClick={handleNewParticipant}>
        ...
        {sub === 'not-found' && (
          <motion.div key="not-found" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-md space-y-6">
            ...
            <p className="text-muted-foreground">
              {searchedId} could not be found. Please check the ID and try again, or register as a new participant.
            </p>
